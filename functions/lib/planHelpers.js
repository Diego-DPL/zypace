...
exports.buildStrengthInstructions = buildStrengthInstructions;
...
exports.buildTrailBlock = buildTrailBlock;
 ...
 */
function buildStrengthInstructions(params) {
...
function validateDayCompliance(plan, runDaysOfWeek, strengthDaysOfWeek, runDaysCount) {
    let violations = 0;
...
// ── Trail block builder ────────────────────────────────────────
function buildTrailBlock(elevationGainM, distKm) {
    const category = elevationGainM >= 5000 ? `Ultra montaña extremo (${elevationGainM}D+) — potencia de subida y control de bajada son las habilidades clave` :
