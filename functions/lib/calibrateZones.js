 ...
 */
function riegel(paceSec, fromKm, toKm) {
...
exports.calibrateZones = (0, https_1.onCall)({ region: 'europe-west1', cors: true, invoker: 'public' }, async (request) => {
    ...
    const { manual_race_km, manual_race_sec } = ((_b = request.data) !== null && _b !== void 0 ? _b : {});
    ...
    for (const doc of actSnap.docs) {
        ...
        if (st && !['run', 'trail', 'walk', ''].some(r => st.includes(r))) {
            if (['ride', 'virtualride', 'swim', 'rowing', 'kayaking', 'ski'].some(s => st.includes(s)))
                continue;
        }
        const dm = Number(a.distance_m) || 0;
        const mt = Number(a.moving_time) || 0;
        if (dm < MIN_DIST || mt < 60)
            continue;
        const distKm = dm / 1000;
        const paceSec = mt / distKm;
        if (paceSec < MIN_PACE || paceSec > MAX_PACE)
            continue;
    ...
    const byPace = (a, b) => a.paceSec - b.paceSec;
    short.sort(byPace);
    medium.sort(byPace);
    long_.sort(byPace);
    // ── 4. Estimate 5k pace ──────────────────────────────────
    ...
    if (!p10kSec && p5kSec)
        p10kSec = riegel(p5kSec, 5, 10);
    if (!p5kSec && p10kSec)
        p5kSec = riegel(p10kSec, 10, 5);
...
//# sourceMappingURL=calibrateZones.js.map