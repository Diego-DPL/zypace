...
export function estimateZones(targetTimeSec: number, distKm: number): TrainingZones | null {
...
export function computePhases(totalWeeks: number): PhaseInfo[] {
  if (totalWeeks >= 16) {
    return [
...
export function phaseForWeek(phases: PhaseInfo[], w: number): PhaseInfo {
  return phases.find(p => w >= p.startWeek && w <= p.endWeek) ?? phases[phases.length - 1];
}

// ── Strength training ─────────────────────────────────────────

export interface StrengthSessionData { desc: string; purpose: string; details: string; }
 ...
 */
export function buildDayScheduleHint(
  startISO: string,
  endISO:   string,
  ...
  if (!hasRunDays && !hasStrDays) return null;
  ...
  while (cur <= end) {
    ...
    let type: string;
    if      (hasRunDays  && runDaysOfWeek!.includes(dow))      type = 'RUNNING';
    else if (hasStrDays  && strengthDaysOfWeek!.includes(dow)) type = 'FUERZA';
    else                                                        type = 'Descanso';
...
export interface FallbackMesocycleParams {
  startISO: string;
  endISO: string;
  totalWeeks: number;        // full plan total weeks (for progression calc)
  mesocycleStartWeek: number; // 1-based week number in full plan
  phases: PhaseInfo[];
  taperWeeks: number;
  runDays: number;
    ...
    for (const wd of Array.from(runDaySet)) {
      if (wd === longRunDay) {
        ...
        dayPlans[wd] = {
          ...
          details: `${km}km en Z1 a ${zones?.z1 || 'ritmo conversacional'}. Mantén una conversación fluida.`,
