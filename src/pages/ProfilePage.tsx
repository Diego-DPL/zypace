...
interface RunnerStats {
  ...
  avgWeeklyKm: number; // last 12 weeks
}
...
const ProfilePage = () => {
  ...
  const [manualDistKm, setManualDistKm] = useState('10');
  ...
  const verifyStravaConnection = useCallback(async () => {
    ...
    try {
      ...
      setIsStravaConnected(response.ok);
    } catch {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      const activities = activitiesSnap.docs
        .map(d => d.data())
        .filter(a => a.type === 'Run' && typeof a.distance === 'number');

      if (!activities.length) return;

      const now = Date.now();
      const ms4weeks  = 28 * 86400000;
      const ms12weeks = 84 * 86400000;

      let totalKm       = 0;
      let last4WeeksKm  = 0;
      let last12WeeksKm = 0;
      let longestKm     = 0;
      let weeksWithRuns = new Set<string>();
      ...
      for (const a of activities) {
        ...
        // Get activity date
        const dateStr: string = typeof a.start_date === 'string'
          ? a.start_date
          : a.start_date?.toDate?.().toISOString() ?? '';
        const actMs = new Date(dateStr).getTime();
        const diffMs = now - actMs;

        if (diffMs < ms4weeks)  last4WeeksKm  += km;
        if (diffMs < ms12weeks) {
          last12WeeksKm += km;
          // Track which week
          ...
          weeksWithRuns.add(mon.toISOString().split('T')[0]);
        }
      }

      const activeWeeks = weeksWithRuns.size || 1;
      const avgWeeklyKm = last12WeeksKm / Math.max(activeWeeks, 4); // normalise to at least 4 weeks

      const lastDateStr: string = typeof activities[0].start_date === 'string'
        ? activities[0].start_date
        : activities[0].start_date?.toDate?.().toISOString() ?? null;

      setStats({
        totalRuns:     activities.length,
        totalKm:       Math.round(totalKm),
        last4WeeksKm:  Math.round(last4WeeksKm),
        longestRunKm:  Math.round(longestKm * 10) / 10,
        lastActivityDate: lastDateStr ? lastDateStr.substring(0, 10) : null,
  ...
  useEffect(() => {
    ...
    loadUserDoc();
    loadStats();
  }, [verifyStravaConnection, loadUserDoc, loadStats]);
  ...
  const handleCalibrateZones = async () => {
    ...
    try {
      ...
      await loadUserDoc();
      setCalibrationMsg({ type: 'success', text: `Zonas calibradas desde ${data.activities_analyzed} actividades. Confianza: ${data.confidence}.` });
    } catch (e: unknown) {
  ...
  return (
    <main className="container mx-auto p-8 space-y-8">
      <h1 className="text-4xl font-bold text-zinc-100">Mi Perfil</h1>
      ...
      {isStravaConnected && (
        <div className="bg-zinc-900 p-6 rounded-xl shadow-lg">
          <div className="flex items-center justify-between mb-4 gap-3">
            ...
            <img src={compatibleWithStrava} alt="Compatible with Strava" className="h-5 w-auto opacity-70" />
          </div>
          {loadingStats ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              <StatCard label="Carreras totales" value={stats.totalRuns.toLocaleString()} />
              <StatCard label="Km totales" value={`${stats.totalKm.toLocaleString()} km`} />
              <StatCard label="Últimas 4 semanas" value={`${stats.last4WeeksKm} km`} />
              <StatCard label="Media semanal (12sem)" value={`${stats.avgWeeklyKm} km`} />
              <StatCard label="Rodaje más largo" value={`${stats.longestRunKm} km`} />
            </div>
          ) : (
            <p className="text-zinc-500 text-sm">Sin actividades de running registradas todavía.</p>
          )}
          {stats?.lastActivityDate && (
            <p className="text-xs text-zinc-600 mt-3">Última actividad: {timeAgo(stats.lastActivityDate)}</p>
          )}
        </div>
      )}

      {/* Strava not connected hint */}
      {!loadingStrava && !isStravaConnected && (
        <div className="bg-zinc-900 p-5 rounded-xl border border-zinc-800 flex items-center gap-3">
          <div className="text-2xl">📊</div>
          <div>
            <p className="text-sm font-semibold text-zinc-200">Sin estadísticas</p>
            <p className="text-xs text-zinc-500 mt-0.5">
              Conecta Strava en{' '}
              <a href="/settings" className="text-lime-400 underline hover:text-lime-300">Ajustes</a>
              {' '}para ver tus estadísticas de running.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-zinc-900 p-6 rounded-xl shadow-lg">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5">
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-1">
              Tus zonas de entrenamiento personalizadas. Se usan automáticamente al generar planes.
            </p>
          </div>
        ...
        {showManualForm && (
          <div className="mb-4 p-4 bg-blue-950/40 border border-blue-800 rounded-xl">
            ...
            <p className="text-xs text-blue-400 mb-3">Tu mejor marca reciente en carrera o test. Más preciso que el análisis de entrenamientos.</p>
        ...
        {hasZones && zoneProfile ? (
          <>
            <div className="flex flex-wrap items-center gap-3 mb-4">
              ...
              {zoneProfile.zones_source === 'strava' && zoneProfile.zones_activities && (
                <span className="text-xs text-zinc-500">{zoneProfile.zones_activities} actividades analizadas</span>
              )}
              {zoneProfile.zones_calibrated_at && (
                <span className="text-xs text-zinc-600">Calibrado {timeAgo(zoneProfile.zones_calibrated_at)}</span>
              )}
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
              <ZoneCard zone="Z1" label="Fácil / Aeróbico" pace={secKmToDisplay(zoneProfile.z1_pace_sec_km!)}
                description="Ritmo conversacional. La base de todo tu entrenamiento (80% del volumen)." color="green" />
              <ZoneCard zone="Z4" label="Umbral / LT2" pace={secKmToDisplay(zoneProfile.z4_pace_sec_km!)}
                description="Ritmo 10k aprox. Sesiones de umbral (noruego) o tempo (polarizado)." color="yellow" />
              <ZoneCard zone="Z5" label="VO2max" pace={secKmToDisplay(zoneProfile.z5_pace_sec_km!)}
                description="Ritmo 5k aprox. Sesiones de intervalos (polarizado, 20% del volumen)." color="red" />
            </div>
            ...
            {(zoneProfile.estimated_5k_sec || zoneProfile.estimated_10k_sec) && (
              <div className="border-t border-zinc-800 pt-4">
                ...
                <div className="flex flex-wrap gap-4">
                  {zoneProfile.estimated_5k_sec && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono font-bold text-zinc-100 text-lg">{totalSecToDisplay(zoneProfile.estimated_5k_sec)}</span>
                    </div>
                  )}
                  {zoneProfile.estimated_10k_sec && (
                    <>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono font-bold text-zinc-100 text-lg">{totalSecToDisplay(zoneProfile.estimated_10k_sec)}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-zinc-500">Media maratón (est.)</span>
                ...
                <p className="text-xs text-zinc-600 mt-2">Estimaciones basadas en tus mejores esfuerzos recientes. No son marcas oficiales.</p>
              </div>
            )}
          </>
        ) : (
          isStravaConnected && !calibrating && (
            <div className="text-center py-8 text-zinc-600">
              ...
              <p className="text-sm mt-1">Pulsa "Calibrar desde Strava" para obtener ritmos personalizados.</p>
            </div>
          )
        )}
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-5">
          ...
          {profileLoaded && (
            <div className="flex items-center gap-2 shrink-0">
              <span className="text-xs font-semibold px-2 py-1 rounded-full bg-lime-400/10 border border-lime-400/20 text-lime-400">
                {experienceLevelLabel[runnerProfile.experience_level]}
              </span>
              <span className="text-xs text-zinc-500">{runnerProfile.age_range} años</span>
            </div>
          )}
          {!profileLoaded && (
            <span className="text-xs text-amber-400 bg-amber-950/40 border border-amber-800 px-2 py-1 rounded-lg">
              Sin perfil guardado — se usarán valores por defecto
            </span>
          )}
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-semibold text-zinc-200 mb-2">
                Km semanales: <span className="text-lime-500 font-bold">{runnerProfile.current_weekly_km} km</span>
              </label>
            ...
            <div>
              <label className="block text-sm font-semibold text-zinc-200 mb-2">
                Rodaje largo: <span className="text-lime-500 font-bold">{runnerProfile.longest_recent_run_km} km</span>
              </label>
            ...
            <div>
              <label className="block text-sm font-semibold text-zinc-200 mb-2">
                Tiempo máx/sesión: <span className="text-lime-500 font-bold">{runnerProfile.max_session_minutes} min</span>
              </label>
