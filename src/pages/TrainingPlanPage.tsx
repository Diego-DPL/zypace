...
const TrainingPlanPage = () => {
  ...
  const handleGeneratePlan = async () => {
    ...
    try {
      ...
      for (const oldPlan of oldPlanSnap.docs) {
        // Delete workouts belonging to old plan
  ...
  const handleRegenerateFromToday = async () => {
    ...
    try {
      ...
      // Save snapshot of current plan version
      await addDoc(collection(db, 'users', user.uid, 'training_plan_versions'), {
        plan_id:       plan.id,
        race_id:       race.id,
        goal:          plan.goal,
                ...
                <div className="space-y-6 mb-8">
                  <fieldset className="border border-orange-200 rounded-lg p-4 bg-orange-50/30">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-2">
                      {([
                        { value: 'polarized', label: 'Polarizado', desc: '80% fácil Z1 · 20% alta intensidad Z4-Z5. Sin zona media. Máxima evidencia científica (Seiler).' },
                        { value: 'norwegian', label: 'Noruego',    desc: '2 sesiones de umbral/semana. Todo lo demás en Z1 estricto. (Bakken / Ingebrigtsen).' },
                        { value: 'classic',   label: 'Clásico',    desc: 'Series martes · Tempo jueves · Largo domingo. Progresión lineal tradicional.' },
                      ] as const).map(m => (
                        <label
                          key={m.value}
