...
const SkeletonCard = () => (
  <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-5 animate-pulse">
    ...
    <div className="h-7 w-24 bg-zinc-700 rounded mb-3" />
    <div className="h-2 w-32 bg-zinc-800 rounded mb-2" />
    <div className="h-8 rounded bg-zinc-800 mt-3" />
  </div>
);
...
const HomePage = () => {
  ...
  const weekStartISO = () => {
    ...
    d.setHours(0, 0, 0, 0); return iso(d);
  };
  const weekEndISO = () => {
    const s = new Date(weekStartISO()); s.setDate(s.getDate() + 6); return iso(s);
  };
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // ── User profile (zones check) ────────────────────────────
      const userSnap = await getDoc(doc(db, 'users', uid));
      setHasZones(!!(userSnap.exists() && userSnap.data().z1_pace_sec_km));

      // ── Next race ──────────────────────────────────────────────
      const racesSnap = await getDocs(
        query(collection(db, 'users', uid, 'races'), where('date', '>=', todayISO), orderBy('date', 'asc'), limit(1))
      );
      ...
      // ── This week's workouts (km stat only) ───────────────────
      ...
      const act7Snap = await getDocs(
        query(collection(db, 'users', uid, 'strava_activities'),
          where('start_date', '>=', since7), orderBy('start_date', 'desc'), limit(5))
      );
      const activities7 = act7Snap.docs.map(d => ({ ...d.data() } as Activity));
      setRecentActivities(activities7);

      // ── Activities & workouts last 28 days ────────────────────
      ...
      // ── Weekly km stats ────────────────────────────────────────
      const planKmWeek = workoutsWeek.reduce((a: number, w: any) => a + (w.distance_km || 0), 0);
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        for (const w of workouts28) {
          ...
          const type = (w as any).explanation_json?.type || '';
      ...
      for (const w of workouts90) {
        if (!(w as any).is_completed) continue;
        const rpe    = typeof (w as any).rpe === 'number' ? (w as any).rpe : 0;
        const durMin = typeof (w as any).duration_min === 'number' ? (w as any).duration_min : 0;
        const km     = ((w as any).distance_km || 0) as number;
        const type   = (w as any).explanation_json?.type || '';
        const zFact  = (type === 'umbral' || type === 'tempo') ? 2.5 : type === 'series' ? 3.5 : 1.0;
        const d      = (w as any).workout_date as string;
        const tl     = (rpe > 0 && durMin > 0) ? durMin * Math.pow(rpe / 10, 2) * 10 : km * zFact;
      ...
      if (ctl >= 0.3) {
        ...
        setFitnessData({
          ctl: parseFloat(ctl.toFixed(1)), atl: parseFloat(atl.toFixed(1)),
          tsb: parseFloat((ctl - atl).toFixed(1)), acwr,
      ...
      if (planId) {
        ...
        const pws   = pwSnap.docs.map(d => d.data());
        const total = pws.length;
        const done  = pws.filter((w: any) => w.is_completed).length;
  ...
  const daysToRace = nextRace
    ? Math.max(0, Math.ceil((new Date(nextRace.date).getTime() - Date.now()) / 86400000))
    : null;

  const sparklinePath = useMemo(() => {
    if (!weeklyTrend.length) return '';
    const max = Math.max(...weeklyTrend, 1); const w = 100; const h = 30;
    return weeklyTrend.map((v, i) => {
  ...
  const contextualTips = useMemo(() => {
    ...
    if (!hasZones) {
      list.push({ icon: '🎯', color: 'text-lime-400', text: 'Calibra tus zonas de ritmo en Perfil para que el plan sea más preciso.' });
    }
    if (daysToRace !== null && daysToRace > 0 && daysToRace <= 7) {
      list.push({ icon: '🏁', color: 'text-amber-400', text: `Faltan ${daysToRace} días para ${nextRace?.name}. Mantén el tapering y descansa bien.` });
    }
    if (nextRace && !planProgress) {
      list.push({ icon: '📋', color: 'text-orange-400', text: `"${nextRace.name}" no tiene plan. Crea uno desde el Calendario.` });
    }
    if (fitnessData?.injuryRisk === 'high') {
      list.push({ icon: '⚠️', color: 'text-red-400', text: `ACWR ${fitnessData.acwr} — reduce el volumen o intensidad esta semana para evitar lesión.` });
    } else if (fitnessData?.injuryRisk === 'moderate') {
      list.push({ icon: '⚡', color: 'text-amber-400', text: `ACWR ${fitnessData.acwr} — carga algo elevada. Prioriza el descanso y evita sesiones extra.` });
    }
    if (fitnessData && fitnessData.tsb > 10 && daysToRace !== null && daysToRace <= 14) {
      list.push({ icon: '🚀', color: 'text-emerald-400', text: 'TSB positivo y estás descansado. Buen momento para competir.' });
    }
    if (recentActivities.length === 0) {
      list.push({ icon: '🔄', color: 'text-sky-400', text: 'Sin actividades recientes. Sincroniza Strava tras tu próxima sesión.' });
    }
    // Fallback if nothing contextual
    if (list.length === 0) {
      list.push({ icon: '⚡', color: 'text-lime-400', text: 'Marca tus entrenos para ver la evolución del plan.' });
      list.push({ icon: '🧠', color: 'text-purple-400', text: 'Lee la explicación de cada workout para entender la intención del entrenamiento.' });
    }

    return list.slice(0, 4);
  }, [hasZones, daysToRace, nextRace, planProgress, fitnessData, recentActivities]);
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-10 pt-8 pb-16">
          ...
          <div>
            ...
            {user && (
              <p className="mt-2 text-sm text-zinc-400">
                Hola <span className="font-semibold text-zinc-100">{firstName || user.email?.split('@')[0]}</span>, este es tu resumen de entrenamiento.
              </p>
            )}
            <button
        ...
        {loading ? (
          <div className="space-y-12">
            ...
            <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-6 animate-pulse h-44" />
            <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-6 animate-pulse h-44" />
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
              <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-6 animate-pulse h-52" />
              <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-6 animate-pulse h-52" />
            </div>
          </div>
        ) : (
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                <div className="rounded-2xl bg-zinc-900/90 h-full p-5 flex flex-col">
                  ...
                  <div className="mt-3">
                    <svg viewBox="0 0 100 30" className="w-full h-8 overflow-visible">
                      ...
                      <defs>
                        <linearGradient id="gradWeek" x1="0" x2="1" y1="0" y2="0">
                          <stop offset="0%" stopColor="#059669" /><stop offset="100%" stopColor="#06b6d4" />
                        </linearGradient>
                      </defs>
                    </svg>
                    <div className="flex justify-between text-[10px] text-zinc-600 mt-1">
                      {weeklyTrend.map((v, i) => <span key={i}>{v}</span>)}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="relative group rounded-2xl p-[1px] bg-gradient-to-br from-indigo-400 via-violet-500 to-fuchsia-600 shadow-lg">
                <div className="rounded-2xl bg-zinc-900/90 h-full p-5 flex flex-col">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-2xl font-bold text-zinc-100">{last28Km.total.toFixed(1)}</span>
                    <span className="text-xs font-semibold text-violet-600">km</span>
                  </div>
                  ...
                  <div className="mt-3 grid grid-cols-4 gap-1">
                    {weeklyTrend.map((v, i) => {
                      const pct = maxWeeklyTrend ? v / maxWeeklyTrend : 0;
                      return (
                    ...
                    <div className="flex items-center gap-4 w-full">
                      <div className="relative w-20 h-20">
                        ...
                        <svg className="absolute inset-0" viewBox="0 0 36 36">
                          ...
                          <defs>
                            <linearGradient id="gradProg" x1="0" x2="1" y1="0" y2="1">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-2 h-1.5 w-full rounded bg-zinc-700 overflow-hidden">
                          <div className="h-full bg-gradient-to-r from-yellow-400 via-lime-400 to-red-500" style={{ width: `${planProgress.percent}%` }} />
                        </div>
                      </div>
                    </div>
                      ...
                      return (
                        <div key={idx}>
                          <div className="flex items-center gap-3">
                            ...
                            <span className={`text-[10px] font-bold w-14 text-right shrink-0 ${onTarget ? 'text-emerald-600' : 'text-amber-600'}`}>{pZ1}% Z1</span>
                          </div>
                ...
                <div className="rounded-2xl bg-zinc-900/90 backdrop-blur-sm p-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                    ...
                    <div className={`rounded-xl border p-3 text-center ${fitnessData.tsb >= 0 ? 'border-emerald-800 bg-emerald-950/40' : 'border-amber-800 bg-amber-950/40'}`}>
                      ...
                      <div className={`text-2xl font-bold ${fitnessData.tsb >= 0 ? 'text-emerald-300' : 'text-amber-300'}`}>{fitnessData.tsb > 0 ? '+' : ''}{fitnessData.tsb}</div>
            ...
            {/* ── Bottom: recent activity + contextual tips ── */}
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
              ...
              <div className="relative group rounded-2xl p-[1px] bg-gradient-to-br from-blue-300 via-cyan-200 to-sky-200 shadow">
                <div className="rounded-2xl bg-zinc-900/90 backdrop-blur-sm p-6">
                  ...
                  {recentActivities.length === 0
                    ? <p className="text-sm text-zinc-500">Sin actividades recientes en Strava.</p>
                    : (
                      <ul className="space-y-3 text-sm">
                        {recentActivities.map(a => (
                          <li key={a.activity_id} className="flex justify-between items-center bg-zinc-800 border border-zinc-700 rounded-xl px-3 py-2">
                            ...
                            <span className="text-xs font-semibold text-sky-400 bg-sky-900/40 px-2 py-1 rounded-full">
              ...
              <div className="relative group rounded-2xl p-[1px] bg-gradient-to-br from-slate-300 via-slate-200 to-slate-300 shadow">
                <div className="rounded-2xl bg-zinc-900/90 backdrop-blur-sm p-6">
                  <h2 className="text-lg font-semibold text-zinc-100 mb-4">Avisos y consejos</h2>
                  <ul className="space-y-4">
                    {contextualTips.map((tip, i) => (
                      <li key={i} className="flex gap-3 items-start text-sm">
                        <span className={`text-base shrink-0 mt-0.5 ${tip.color}`}>{tip.icon}</span>
