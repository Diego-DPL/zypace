    ...
    <header className="bg-zinc-950/90 backdrop-blur supports-[backdrop-filter]:bg-zinc-950/80 border-b border-zinc-800 sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm font-medium">
          <Link to="/calendar" className={navLink('/calendar')}>Calendario</Link>
          <Link to="/profile" className={navLink('/profile')}>Perfil</Link>
          <Link to="/settings" className={`${navLink('/settings')} flex items-center gap-1.5`}>
            Ajustes
        ...
        <div className="hidden md:flex items-center gap-4">
          <span className="text-zinc-500 text-sm max-w-[140px] truncate" title={user?.email ?? undefined}>Hola, {displayName}</span>
          <button
      ...
      <div className={`md:hidden overflow-hidden transition-[max-height] duration-300 ease-in-out border-t border-zinc-800 ${open ? 'max-h-96' : 'max-h-0'}`}>
        <div className="px-4 pt-2 pb-4 space-y-4 text-sm bg-zinc-950">
          <div className="flex flex-col gap-2">
            <Link onClick={() => setOpen(false)} to="/calendar" className={`py-2 px-3 rounded hover:bg-zinc-800 ${location.pathname === '/calendar' ? 'text-lime-400 font-semibold' : 'text-zinc-300'}`}>Calendario</Link>
            <Link onClick={() => setOpen(false)} to="/profile" className={`py-2 px-3 rounded hover:bg-zinc-800 ${location.pathname === '/profile' ? 'text-lime-400 font-semibold' : 'text-zinc-300'}`}>Perfil</Link>
            <Link onClick={() => setOpen(false)} to="/settings" className={`py-2 px-3 rounded hover:bg-zinc-800 flex items-center gap-2 ${location.pathname === '/settings' ? 'text-lime-400 font-semibold' : 'text-zinc-300'}`}>
              Ajustes
              {hasBillingIssue && <span className="w-1.5 h-1.5 rounded-full bg-yellow-400" />}
            </Link>
            {role === 'admin' && (
